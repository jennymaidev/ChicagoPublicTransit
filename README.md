...
To visualize this relationship, we generated a scatter plot of minimum distance versus sale price with a fitted linear regression line (Figure 1). The scatter plot demonstrates the weak relationship visually: while a downward trend is apparent, substantial scatter around the regression line illustrates the limited predictive power of distance alone. Properties immediately adjacent to stations show considerable price variation, indicating that factors beyond transit proximity substantially influence property values.
...
We calculated separate Pearson correlations between binary indicators of specific CTA lines and property sale price. The results were striking: all line-specific correlations were negligible, with absolute values less than 0.005. The Red Line showed a correlation of r ≈ -0.004 with sale price, while other lines ranged from r ≈ -0.003 (Green Line) to r ≈ 0.0005 (Blue Line). The number of transit lines serving a station (a proxy for hub importance) showed an equally negligible correlation (r ≈ -0.0002) with sale price.

This null finding is notable and contrasts with our initial hypothesis. While the Red Line does serve downtown Chicago and major employment centers, proximity to the Red Line specifically confers no measurable price advantage relative to properties near other transit lines. Similarly, properties near multi-line transit hubs show no meaningful premium compared to single-line stations. The absence of line-specific effects suggests that the transit premium observed in RQ1 is driven by general accessibility to public transit infrastructure rather than by the specific characteristics of individual lines (capacity, coverage area, employment accessibility).

To quantify the explanatory power of specific line identity, we conducted preliminary analysis predicting sale price from binary indicators of nearest line color. Line identity as a sole predictor contributed negligible explanatory power beyond chance, indicating that while transit proximity matters for property values, the specific line serving a property does not. This finding has important implications for urban planning: transit investments in peripheral lines may yield comparable property value benefits per kilometer of service to investments in high-capacity lines, suggesting that equitable transit expansion may not disadvantage underserved areas in terms of property value capitalization.
...
We first assessed collinearity between property class and distance using Pearson correlation. The resulting correlation coefficient was r ≈ -0.01 (p < 0.001), indicating negligible relationship. This finding confirms that properties of different classes are distributed relatively uniformly across distance bands, eliminating collinearity concerns from regression estimation.

We then conducted ordinary least squares regression predicting sale price from a model including main effects for class and distance, as well as their interaction term: sale_price ~ class + distance + class × distance. The results are presented in Table 3. The full model achieved R² = 0.392, indicating that property class and distance jointly explain 39.2% of the variance in property sale prices. This represents a substantial improvement over the 5.2% explained by distance alone, indicating that property type is a critical price-determining factor.
...
The main effect coefficient for property class was highly significant (p < 0.001), indicating that structural characteristics (captured imperfectly by the three-digit class code) significantly influence price. Larger homes command substantial premiums relative to smaller homes. The interaction term (class × distance) was statistically significant (p < 0.001), indicating that the strength of the distance-price relationship varies by property type. However, the small magnitude of the interaction coefficient suggests that while these variations exist, they are modest in practical terms.
...
To further assess the relationships between predictor variables, we generated a correlation matrix visualization (Figure 4) examining the correlations between property class, distance to nearest CTA station, and sale price. This assessment confirmed low correlation between class and distance (r ≈ -0.01), validating the assumption of minimal multicollinearity in the regression model.
