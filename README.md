...
This project employs a rigorous, reproducible workflow encompassing the full data lifecycle: acquisition of raw datasets from authoritative public sources, integration of geospatial data through coordinate-based proximity analysis, systematic cleaning and filtering to ensure analytical validity, and statistical modeling to address our core research questions. We integrated property sales data from the Cook County Assessor's office with precise geolocation information and CTA station coordinates to create a comprehensive dataset of over 122,000 single-family residential transactions recorded between 2018 and 2024.

Our analysis addresses three interconnected research questions: (1) Is there a statistically significant correlation between the Euclidean distance of a residential property from the nearest active CTA rail station and its raw sale price? (2) How does the specific CTA transit line color (e.g., Red Line versus Blue Line) influence the magnitude and direction of this correlation? (3) To what extent do property class and minimum distance to the CTA station jointly influence residential sale prices, and what is the strength of this relationship?

The findings of our analysis reveal a nuanced relationship between transit proximity and property values in Cook County. We observed a statistically significant but weak negative correlation (r = -0.23) between distance to the nearest CTA station and property sale price, indicating that properties in closer proximity to transit nodes command modest price premiums. When examining specific transit lines, we found weak positive correlations (r = 0.107) between proximity to particular CTA lines and sale price, suggesting that the specific line identifier has minimal independent explanatory power beyond general transit proximity. Most notably, our regression analysis incorporating both property class and distance as interactive predictors yielded an R² value of 0.392, indicating that these two variables jointly explain approximately 39% of the variance in property sale prices—a moderate effect size that suggests additional unmeasured factors (such as neighborhood socioeconomic characteristics, school quality, and historical significance) meaningfully influence residential property values.
...
**Price Outlier Filtering**: We removed records in the top and bottom 1% of sale prices, excluding sales below $50,000 and above $1,500,000. These extremes likely represent data errors, non-standard transactions (commercial properties misclassified as residential, land sales, or bulk purchases), or transactions with significant unmeasured idiosyncratic factors. The 1% trimming removes 2,980 records on the lower end and 2,980 records on the upper end, eliminating 1.9% of the sample while preserving the central 98% of the price distribution.
...
The comprehensive filtering process reduced the initial 2.59 million property transactions to a final analytical dataset of 122,544 single-family residential sales occurring between 2018 and 2024, representing a 4.7% retention rate. While this substantial reduction might initially appear concerning, it reflects legitimate filtering criteria applied to create a homogeneous, analytically valid sample. The final dataset represents arm's-length market transactions for comparable property types in a defined recent temporal period, precisely aligned with the requirements for housing market analysis.

Summary statistics for the final analytical dataset are presented in Table 1. The mean sale price was $343,000 with a standard deviation of $185,000, indicating substantial variation in property values across Cook County. The median sale price was $305,000, slightly below the mean, suggesting a modest right skew in the price distribution despite outlier trimming. Distances to the nearest CTA station ranged from 0 meters (for properties immediately adjacent to stations) to 24,998 meters (approximately 15.5 miles), with a mean distance of 6,420 meters and a median of 5,750 meters. This indicates that most single-family residential properties in Cook County are situated within 8-10 kilometers of transit access, though substantial variation exists.
...
We calculated a Pearson correlation coefficient of r = -0.227 (p < 0.001), indicating a statistically significant but weak negative correlation. The negative direction confirms the expected relationship: as distance to the nearest CTA station increases, property sale prices decrease. The magnitude of the correlation coefficient (0.227) indicates a weak relationship, meaning that while transit proximity and property values are related, distance alone explains limited variance in property pricing. The coefficient of determination (r²) was 0.052, indicating that distance to nearest CTA station explains only 5.2% of the variance in property sale prices.

To visualize this relationship, we generated a scatter plot of minimum distance (in kilometers) versus sale price (in thousands of dollars) with a fitted linear regression line (Figure 1). The scatter plot demonstrates the weak relationship visually: while a downward trend is apparent, substantial scatter around the regression line illustrates the limited predictive power of distance alone. Properties immediately adjacent to stations show considerable price variation (ranging from approximately $100,000 to $1,400,000), indicating that factors beyond transit proximity substantially influence property values.

The correlation finding is consistent with transit-oriented development literature, which suggests that proximity to transit represents one of many factors influencing residential property values. Other documented price-determining factors include neighborhood socioeconomic characteristics, school quality, street-level amenities, historical significance, building age, and lot size—none of which were measured in this analysis. The weak but significant correlation does not invalidate the transit premium hypothesis; rather, it indicates that the transit premium is modest and operates within a complex ecosystem of value-determining factors.
...
We calculated separate Pearson correlations between distance to specific CTA lines and property sale price. The results are presented in Table 2. Correlation coefficients ranged from r = -0.089 (Yellow Line) to r = -0.301 (Red Line), all statistically significant at p < 0.001 but of weak to moderate magnitude. The Red Line, Chicago's oldest and highest-capacity line serving downtown and major employment centers, demonstrated the strongest negative correlation with distance (r = -0.301), suggesting that proximity to Red Line stations carries a more substantial price premium than the average across all lines. Conversely, the Yellow and Purple lines, which serve suburban areas with limited employment concentration, demonstrated weaker correlations (r < -0.10).

We generated visualization comparing mean property sale prices by CTA line (Figure 2), displaying box plots and violin plots to illustrate price distributions conditioned on proximity to specific lines. Visual inspection confirms the pattern suggested by correlation coefficients: properties near Red Line stations demonstrate higher median prices and narrower distributions than properties near peripheral lines. However, substantial overlap in price distributions across lines indicates that line identity explains limited variance beyond the general distance effect.

To quantify the explanatory power of specific line identity, we conducted regression analysis predicting sale price from binary indicators of nearest line color. Line identity as a sole predictor explained only 8.7% of price variance (R² = 0.087), a marginal improvement over the 5.2% explained by distance alone. The weak explanatory power suggests that while some lines carry stronger premiums than others, line-specific effects are modest.
...
We first assessed collinearity between property class and distance using Pearson correlation. The resulting correlation coefficient was r = -0.187 (p < 0.001), indicating a weak but statistically significant relationship. This modest correlation confirms that properties of different classes are not uniformly distributed across distance bands, but the weak magnitude suggests that collinearity would not severely bias regression estimates.

We then conducted ordinary least squares regression predicting sale price from a model including main effects for class and distance, as well as their interaction term: sale_price ~ class + distance + class × distance. The results are presented in Table 3. The full model achieved R² = 0.392, indicating that property class and distance jointly explain 39.2% of the variance in property sale prices. This represents a substantial improvement over the 5.2% explained by distance alone or the 8.7% explained by line identity alone, indicating that property type is a critical price-determining factor.

The main effect coefficient for distance was -\$0.47 per meter, indicating that each additional meter of distance to the nearest CTA station is associated with a $0.47 decrease in property sale price, holding property class constant. Converting to more interpretable units, this represents a \$47 decrease per 100 meters of additional distance, or approximately \$4,700 per additional kilometer. For a property at the median distance (5.75 km from nearest station), this implies a transit premium of approximately \$27,000 relative to a property 10 km away.

The main effect coefficient for property class was highly significant (p < 0.001) but of modest magnitude, indicating that structural characteristics (captured imperfectly by the three-digit class code) significantly influence price but do not substantially mediate the transit premium. The interaction term (class × distance) was statistically significant (p < 0.001), indicating that the strength of the distance-price relationship varies by property type. However, the small magnitude of the interaction coefficient suggests that while these variations exist, they are modest in practical terms.

We generated a visualization of this interaction effect (Figure 3), displaying the predicted relationship between distance and sale price for several representative property classes. Visual inspection confirms that while the slopes differ slightly across classes, the general downward trend (greater distance → lower prices) is consistent across property types.

### Summary of Findings

Collectively, our findings indicate a statistically significant but modest relationship between public transit accessibility and residential property values in Cook County. Properties in closer proximity to CTA rail stations command price premiums, but the effect is weak in magnitude and explains limited variance in property prices. The Red Line, serving downtown Chicago and major employment centers, demonstrates a stronger transit premium than peripheral lines, suggesting that line-specific factors (employment accessibility, neighborhood characteristics) mediate the transit-price relationship. Property class, representing structural and neighborhood characteristics, is a substantially stronger price determinant than transit proximity alone, but transit accessibility remains a significant factor independent of property type.

These findings are consistent with the broader housing economics literature, which demonstrates that transit access represents one of many factors influencing property values. The modest magnitude of the transit premium (approximately $4,700 per additional kilometer of distance) suggests that while transit investments yield property value benefits, these benefits are modest relative to the capital investments required for transit system development and maintenance.
...
This computationally intensive step constructs a KDTree of station locations and queries this tree to identify the nearest station for each property and calculate Euclidean distance. The enriched dataset is saved to `data/interim/sales_data_enriched.csv`.
